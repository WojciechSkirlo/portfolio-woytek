...
<template>
    ...
    >
        <div
            class="flex items-center justify-center w-full h-full rounded-lg bg-accent-light"
        ...
        >
            <div class="flex flex-col items-center text-center">
                <div
                    ...
                    :class="
                        ((item.hasOwnProperty('live') && item.live) ||
                            (item.hasOwnProperty('github') && item.github)) &&
                        'mb-8'
                    "
                >
                    <span
            ...
            <div class="absolute bottom-0 left-0 flex flex-wrap w-full p-4 gap-1.5">
                <template v-if="item.hasOwnProperty('techStack') && item.techStack">
                    <div
                        v-for="(element, index) in item.techStack"
                        :key="index"
                        class="text-white px-1 py-0.5 uppercase font-medium text-xs bg-black"
                    >
                        {{ element }}
                    </div>
                </template>
